...
function getSubjectGroup(name) {
  ...
  c = c.replace(/([가-힣])\s*I\b/g,'$1Ⅰ').replace(/([가-힣])\s*II\b/g,'$1Ⅱ')
       .replace(/([가-힣])1$/,'$1Ⅰ').replace(/([가-힣])2$/,'$1Ⅱ');
  if (S[c]) return S[c].교과군;
  ...
  if (/가정|생활과학|아동/.test(name)) return '기술·가정/정보';
  if (/체육|스포츠/.test(name)) return '체육';
  if (/음악|미술|연극|예술|미디어/.test(name)) return '예술';
  if (/역사|지리|윤리|경제|법과|정치|국제|사회/.test(name)) return '사회';
...
const COMMON_CSS = `
  * { box-sizing: border-box; margin: 0; padding: 0; }
  body { font-family: 'Pretendard','Apple SD Gothic Neo','Noto Sans KR',sans-serif; background:#f0f4f8; color:#0f172a; }
  ...
  .info-card p { font-size:.88rem; line-height:1.75; color:#1f2937; white-space:pre-line; }
  .univ-grid { display:flex; flex-wrap:wrap; gap:.4rem; }
  .univ-chip { background:#f7fafc; border:1px solid #e2e8f0; border-radius:6px; padding:.25rem .6rem; font-size:.75rem; color:#1f2937; }
  ...
  .th-group { background:#f7fafc; color:#1f2937; width:88px; }
  ...
  .td-group { font-size:.73rem; font-weight:700; color:#1f2937; background:#f7fafc; white-space:normal; word-break:keep-all; line-height:1.4; }
  .td-chips { }
  .chips-wrap { display:flex; flex-wrap:wrap; gap:.3rem; align-content:flex-start; }
  .subj-chip { background:#edf2f7; color:#0f172a; font-size:.75rem; padding:.2rem .6rem; border-radius:99px; text-decoration:none; border:1px solid #e2e8f0; cursor:pointer; transition:background .15s; display:inline-block; white-space:nowrap; }
  @media(max-width:600px){ .subj-table { font-size:.72rem; } .th-group { width:52px; } .subj-chip { white-space:normal; word-break:keep-all; font-size:.68rem; padding:.15rem .4rem; } } .subj-chip { white-space:normal; word-break:keep-all; font-size:.7rem; padding:.15rem .4rem; } .td-chips { overflow:hidden; } .th-group { width:56px; } }
  .subj-chip:hover { background:#2d6a9f; color:white; border-color:#1e3a6e; }
...
.dept-header .meta { font-size:.83rem; color:#166534; margin-top:.3rem; }
.info-card h3 { color:#166534; }
.similar-chip { background:#f0fff4; border:1px solid #9ae6b4; color:#14532d; }
...
<div class="page-content">
  ...
  <div class="info-card">
    <h3>🏛️ 개설 대학</h3>
    ${info.개설대학_서울 ? `<p style="font-size:.75rem;font-weight:700;color:#1f2937;margin-bottom:.4rem;">서울</p><div class="univ-grid" style="margin-bottom:.8rem;">${univHTML(info.개설대학_서울)}</div>` : ''}
    ${info.개설대학_수도권 ? `<p style="font-size:.75rem;font-weight:700;color:#1f2937;margin-bottom:.4rem;">수도권</p><div class="univ-grid" style="margin-bottom:.8rem;">${univHTML(info.개설대학_수도권)}</div>` : ''}
    ${info.개설대학_지방 ? `<p style="font-size:.75rem;font-weight:700;color:#1f2937;margin-bottom:.4rem;">지방</p><div class="univ-grid">${univHTML(info.개설대학_지방)}</div>` : ''}
  </div>` : ''}
...
.series-header p { font-size:.85rem; color:#1f2937; margin-top:.2rem; }
.info-card h3 { color:#1e3a6e; }
.dept-chips { display:flex; flex-wrap:wrap; gap:.5rem; }
.dept-chip { background:#e6f0fb; color:#1e3a6e; font-size:.78rem; padding:.3rem .8rem; border-radius:99px; border:1px solid #b9d1f0; text-decoration:none; display:inline-block; }
...
.tip-box h3 { color:#92400e; font-size:.8rem; font-weight:700; margin-bottom:.6rem; }
.tip-box p { font-size:.85rem; color:#4a2c0a; line-height:1.7; white-space:pre-line; }
</style>
</head>
<body>
<div class="top-bar">
...
.subj-hero .intro { font-size:.9rem; color:#4a2c0a; line-height:1.65; margin-top:.5rem; }
...
.meta-type-other    { background:#f3f4f6; color:#1f2937; }
.meta-suneung-y { background:#fef3c7; color:#92400e; }
.meta-suneung-n { background:#f3f4f6; color:#1f2937; }
...
.domain-chip { background:#f7fafc; color:#1f2937; font-size:.78rem; padding:.25rem .7rem; border-radius:6px; border:1px solid #e2e8f0; }
.rec-list { list-style:none; }
.rec-list li { font-size:.88rem; color:#1f2937; line-height:1.7; padding:.2rem 0 .2rem 1.2rem; position:relative; }
.rec-list li::before { content:'✔'; position:absolute; left:0; color:#c05621; font-size:.75rem; top:.25rem; }
...
.suneung-item .s-label { font-size:.7rem; color:#1f2937; margin:.3rem 0 .1rem; }
  ...
  <div class="info-card">
    ...
    <div class="suneung-box">
      <div class="suneung-item">
        ...
        <div class="s-val" style="color:${info.수능출제==='O'?'#c05621':'#4a5568'}">${suneungText}</div>
      </div>
      <div class="suneung-item">
